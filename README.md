...
* **가설 : 음주는 간수치 특성이 가장 영향이 있을 것, 가설은 틀렸나???
    ...
    * => 분석관점보다는 모델 성능에 중점으로 진행했던 프로젝트여서 데이터 구조 파악 및 분석 내용이 많이 부실함
    * => 그렇기 때문에 특정 데이터에서 어떤 형태의 분포를 보이는지에 대한 설명력 부족, 이는 가설을 뒷받침하기 어려움
    * => 현재 데이터상 성비율과 흡연여부의 데이터가 많이 타겟에 편향되어 있었기 때문에 Target에 큰 영향이 컸을 가능성이 있음 So, 가설이 틀렸다고만 판단할 순 없음

* 결과적으로 모델 성능면에서는 좋다고 판단이 되나 분석면에서는 설명력 부족으로 결과에 대한 신뢰도가 떨어진다.
        * = 더 면밀한 데이터 분석이 필요함
