...
* 따라서, 음주 여부(=Target)에 대해 어떤 특성이 영향을 끼치는지 데이터 통계 및 가시화를 통해 분석하고자 한다.
...
### Benefit
* 환자의 건강 상태에 따른 원할한 피드백과 성공적인 수술에 기여
* 평소 식습관, 정신 상태, 수면 습관 등 다양한 방면으로 데이터를 수집하고 특정 타겟을 예측할 수 있는 모델을 구현한다면 국민의 보건의료 수준과 삶의 질을 항샹시키는데 큰 기여를 할 수 있을 것
...
* Data Visualization
    ...
    * seaborn - 데이터 분포 확인
        ...
        - 헤모글로빈(= hemo)의 경우는 술을 안마시는 사람보다 마시는 사람의 값이 높음 즉, Target(=음주여부)의 유효한 Feature
...
![Process (1)](https://user-images.githubusercontent.com/83389640/177258062-4ef89597-221f-4fc4-a4c3-cb498a2aae80.jpg)
    ...
    * 간수치 특성이 타겟에 많은 영향이 있다는 것을 확인(= ALT도 순열중요도에서 파악했을 때 기여함을 학인)
    * => 분석관점보다는 모델 성능에 중점으로 진행했기 때문에 데이터 구조 파악 및 분석 내용이 많이 부족함
...
* **<개선점>**
    ...
    - **모델 성능 82%???**
        ...
        * 각 Feature 마다 이상치가 존재하지만 어디까지 이상치로 보고 어느 부분을 학습 데이터로 사용할건지 정해야함
            = 이상치가 들어왔을 때 오판단 확률이 높을 것
...
* **<어려웠던 점>**
    ...
    - 데이터를 어디까지 봐야하는지에 대한 기준도 없었고 기술적인 역량 또한 많이 부족했음
        - 현재 데이터는 어떤 양상을 보이는지 면밀히 보지 못한 아쉬움이 큼
        - 데이터에서도 어느 부분의 데이터를 학습을 시킬건지에 대한 기준을 잡을 필요가 있음
        - [Comento 빅데이터 직무부트캠프 수강(노션)](https://www.notion.so/Hello-I-m-Kimmy-18a3274b509041088892b3b4f8ff9065)
* **<느낀점>**
    ...
    - 하지만 데이터 분석관점보다는 예측 모델의 성능을 중점으로 진행했기 때문에 앞서 정했던 가설과 목적에 대한 설명력 부족으로 모호한 결과를 가져옴
        - **<"모델을 위한 분석이 아닌 분석을 위한 모델링과 방법론이 되어야한다"는 것을 느낌>**
